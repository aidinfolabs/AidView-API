...
import module namespace iati-sum = "http://kitwallace.me/iati-sum" at "iati-sum.xqm";
...
declare function iati-wo:activity-group-summary ($activities, $query) {
   iati-sum:activities($activities, $query/start-date , $query/end-date,
...
group $activity as $group-activities by $activity/recipient-country/@iati-ad:country as $group-code
return
let $e := error((),"x",$group-code)
let $code := iati-c:code-value($facet,$group-code,$query/corpus)
  ...
  then
     ...
     return
      element iati-activity {
         ...
         iati-sum:activities($activity,$query/start-date, $query/end-date,$query/transaction)
      }
