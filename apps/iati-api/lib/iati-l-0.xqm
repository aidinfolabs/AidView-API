      ...
      return 
          ...
          then update insert $ckan-activitySet into $activitySets
               ...
               then  let $replacement :=
                          element activitySet {
                              $ckan-activitySet/* ,
                              $current-activitySet/download_modified
                          }
...
declare function iati-l:store-activity($activitySet as element(activitySet), $corpus as xs:string, $activity as element(iati-activity)) {
    ...
    let $store := xmldb:store($activity-collection,concat($filename,".xml"),$activity)
    let $stored-activity := collection($activity-collection)/iati-activity[iati-identifier = $id]
...
declare function iati-l:remove-activities ($activitySet as element(activitySet),$corpus as xs:string) {
      ...
      let $activities := collection($activity-collection)/iati-activity[@iati-ad:activitySet=$id]
      let $deletions :=
         for $activity in $activities
         let $file := util:document-name($activity)
         let $delete  := xmldb:move($activity-collection, $dump, $file)
 (:     let $delete  := xmldb:remove($activity-collection, $file) :)
         return $file
      return 
         if (exists($deletions))
        ...
        {for $activitySet in $activitySets
         ...
         return 
            <tr>
              ...
              <td>{count(collection($activity-collection)/iati-activity[@iati-ad:activitySet=$source])}</td>
            </tr>
        }
       </table>
</div>
