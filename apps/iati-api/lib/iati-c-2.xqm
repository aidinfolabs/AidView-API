(:
  this module deals with accessing, loading and  caching codelists
...
declare function iati-c:missing-code-analysis($activities as element(iati-activity)*, $query as element(query) ) as element(div) {
       <div> 
          <table border="1" class="sortable">
               <thead>
                 <tr><th>Code</th><th>Name</th><th>Path</th><th>Faulty Activities</th><th/></tr>
               </thead>
               <tbody>
               ...
               { let $paths := $iati-b:paths/path[@code]
                 ...
                 let $exp := concat("$activities/",$path)
                 let $all-codes := eval($exp)
                 let $missing-codes  :=  for $val in $all-codes where empty(iati-c:code-value($name,$val)) return $val
                 order by $name
                 return 
                  <tr> {if (count($missing-codes) = 0) then attribute class {"good"} else () }
                      <td>{$code}</td>
                      <td>{$name}</td>
                      <td>{$path/string()}</td>
                      <td>{count($missing-codes)}</td>
                      <td>
                      <table>
                        {for $val in $missing-codes
                         let $bad-activities := eval(concat("$activities[",$path," = $val]"))
                         return
                         <tr>
                             <td>{$val}</td>
                             <td>
                              {for $activity in $bad-activities
                               return <a href="?corpus={$query/corpus}&amp;type=activity&amp;id=$activity/iati-identifier">{$activity/iati-identifier}</a>
                              }
                            </td>
                          </tr>
                         }
                         {
                         let $bad-activities := eval(concat("$activities[",$path," = '']"))
                         return
                         <tr><td>blank string</td>
                             <td>
                              {for $activity in $bad-activities
                               return <a href="?corpus={$query/corpus}&amp;type=activity&amp;id=$activity/iati-identifier">{$activity/iati-identifier}</a>
                              }
