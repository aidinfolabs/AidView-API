...
let $cache := doc(concat($iati-b:base,"olap/",$query/corpus,"-stats.xml"))/dimensions
return 
(: --------- type=host  --------------------- :)
...
then
                 ...
                 <tr><th>Code</th><th>Name</th><th># Activities</th></tr>
                  {for $ref in distinct-values($activities//@iati-ad:funder)
                   ...
                   return <tr>
                          ...
                          <td>{count($activities[participating-org/@iati-ad:funder=$ref][@iati-ad:live])}</td>
                         </tr>
                  }
                </table>
               </div>
 ...
 then 
     let $stats := $cache/Funder[code=$query/funder]
     ...
     return  <div>
               ...
               <table>
                  ...
                  <tr><th>Included</th><td>{$stats/included/string()}</td></tr>
                  <tr><th>Activities with locations </th><td>{$stats/with-location/string()}</td></tr>
                  <tr><th>Activities with DAC sectors </th><td>{$stats/with-DAC-sectors/string()}</td></tr>
                  <tr><th>Activities with results </th><td>{$stats/with-result/string()}</td><td>{round($stats/with-result div $count * 100 )}</td></tr>
                  <tr><th>Activities with documents </th><td>{$stats/with-document/string()}</td><td>{round($stats/with-document div $count * 100 )}</td></tr>
                  <tr><th>Activities with conditions </th><td>{$stats/with-conditions/string()}</td><td>{round($stats/with-conditions div $count * 100 )}</td></tr>
               </table>
               <h2>Financial Summary - all values in USD (2010)</h2>
              ...
              {for $type in iati-c:codelist("ValueType")/*
               let $total := sum($stats/summary[@code=$type/code]/@USD-value)
               where $total ne 0
               return 
                 <tr>
                   <td>{$type/code/string()}</td><td>{$type/name/string()}</td>
                   {for $year in $years 
                    let $summary := $stats/summary[@code=$type/code][@year=$year]
                    return
                       <td>{iati-h:print-value($summary/@USD-value)}</td>
                   }
