...
declare function iati-wo:query-activities ($query as element(query) ) as element(activity)* {
  ...
  then concat($filters,"[matches((sector|title|description),'",$query/search,"')]") else $filters
...
let $selected-activities :=
      if (exists($query/groupby) or $activity-count > $max-activities)    (: ignore  the  activities :)
      then ()
      else 
