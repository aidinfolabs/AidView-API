...
declare variable $iati-wo:version := "2011-11-26T17:00:00";
...
declare function iati-wo:query-activities ($query as element(query) ) as element(activity)* {
  ...
  let $filters := if ($query/search) 
  then concat($filters,"[matches((sector|title|description),'",replace($query/search,"'","''"),"')]") else $filters
  let $exp := concat("collection('",$iati-b:data,$query/corpus,"/activities')/iati-activity[@iati-ad:live][@iati-ad:include]",string-join($filters,""))
...
declare function iati-wo:group-by-all($query  as element(query),$activities as element(iati-activity)*)  {
let $facet := "All"
return 
   element {$facet} {
             (
            iati-wo:activity-group-values($activities),
            if ($query/result eq "summary")
            then iati-wo:activity-group-summary($activities,$query)
             else ()
            )
       }
};
...
declare function iati-wo:query-olap($query) {
...
then 
  let $selected-groups := 
      if (empty($query/groupby)) then ()
         ...
         let $selected-groups := iati-wo:query-olap($query)
         return 
            if ($selected-groups) 
            then $selected-groups 
              ...
              return
                  ...
                  then iati-wo:group-by-all($query,$selected-activities)
  ...
  let $selected-activities :=
      ...
      else  iati-wo:query-activities($query) 
...
element result {
 ...
 then
     ...
     return
      element iati-activity {
         $activity/
         (iati-identifier,title[1],reporting-org,participating-org, 
