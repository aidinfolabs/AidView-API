...
return
   element iati-activity {
     ...
     (: make as included if activity-status not 4 or 5  sneeked in here - doesnt really belong:)
      if ($activity/activity-status = ("4","5")) 
      then ()
      else attribute iati-ad:include {}, 
      $activity/*
     }
};
         ...
         case element(value) return
            ...
            let $transaction-type := 
                  typeswitch ($transaction) 
                     case element(transaction) return ($node/../transaction-type/@code,$node/../activity-type/@code)[1]
...
declare function iati-t:add-org-key($activity as element(iati-activity) ) as element (iati-activity)? {
    element iati-activity {
       $activity/@*,
       for $org in $activity/reporting-org
       return 
          element reporting-org {
          org/@*,
          if (empty($activity/participating-org[@role="Funding"]))
          then attribute iati-ad:funder {$org/@ref} 
          else (),
          $org/node
       },
       ...
       return 
        ...
        $activity/(* except ( participating-org, reporting-org))
   }
};
