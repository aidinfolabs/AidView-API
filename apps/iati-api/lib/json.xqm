(:~
 ...
 :    CW  2011-11-10  characters need escaping are " , tab, newline and return 
 :)
module namespace json="http://kitwallace.me/json";
...
declare function json:escape-text($text) {
...
declare function json:element-helper($attribs as attribute()*, $children as node()*) {
    string-join((
        ...
        if ($children instance of text()+) then
            ...
            let $etext := json:escape-text($text)
            return
                concat('"#text": "', $etext, '"')
        else
