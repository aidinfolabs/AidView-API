...
(:
else if (exists($query/corpus) and $query/type="host"  and empty($query/host))
then 
...
(: ------  type = code - master ---------------- :)
   ...
   then 
      ...
      then  
         ...
         return response:redirect-to(xs:anyURI("admin-2.xq?mode=view&amp;type=code"))
 ...
 (:  --------- code and corpus -------------  :)
    ...
    then 
      ...
      then  
         ...
         return response:redirect-to(xs:anyURI(concat("admin-2.xq?corpus=",$query/corpus,"&amp;type=code")))
      ...
      then let $cache := iati-c:cache-organisation-identifier-codes($query/corpus)
        return  response:redirect-to(xs:anyURI(concat("admin-2.xq?corpus=",$query/corpus,"&amp;type=code&amp;id=OrganisationIdentifier")))
