...
module namespace api = "http://kitwallace.me/api";
...
declare variable $api:parameters :=  doc(concat($config:system,"woquery.xml"))//param;
declare variable $api:version := "2011-11-26T17:00:00";
...
declare function api:query-activities ($query as element(query) ) as element(activity)* {
  ...
  then concat($filters,"[matches((sector|title|description),'",replace($query/search,"'","''"),"')]") else $filters
...
declare function api:activity-group-values ($activities) {
...
declare function api:activity-group-summary ($activities, $query) {
...
return 
   element {$facet} {
             (
            api:activity-group-values($activities),
...
let $code := codes:code-value($facet,$group-code,$query/corpus)
return 
   element {$facet} {
       ...
       if ($query/result eq "ids") then ()
           ...
           return
            (
            api:activity-group-values($group-activities),
            if ($query/result eq "summary")
...
for $group-code in $group-codes
let $code := codes:code-value("Funder",$group-code,$query/corpus)
return 
   element {$facet} {
       element code {$group-code},
       element name {$code/name/string()},
...
let $olap := doc(concat($config:base,"olap/", $query/corpus,".xml"))/dimensions
return
...
declare function api:api-help($query) {
<html>
   <head>
    ...
    <body>
       ...
       <tr><th>corpus</th>
            ...
            <td>  
                ...
                {for $corpus in doc(concat($config:system,"corpusindex.xml"))//corpus[empty(@hidden)]/text()
                 return 
                    element option {
      ...
      <ul>
         ...
         <li><s>index tuning - may need to add more in the activity transformation</s></li>
         <li>full corpus required for stress testing </li>
         <li><s>new result type to include locations TBD  - result - geo</s> </li>
         <li><s>transaction summary result type TBD  - result summary + transaction and date range</s></li>
         <li><s>date filter TBD  = range for transactions </s></li>
      </ul>
  </body>
 </html>
};
...
then 
  ...
  let $selected-activities :=
      if ($activity-count = 0 or ($activity-count > $max-activities and empty($query/pagesize)) ) then ()   (: then sort the  activities :)
      else 
...
element result {
 ...
 then
  ...
  then   
     let $geo-locations := collection(concat($config:base,"geo"))
     for $activity in $selected-items
...
(:     
let $json := util:serialize($result,"method=json media-type=application/json"  

- preferable but unusable - returns two copies  see bugtracker

 - but json serialzation would probably be OK for straight json - needs testing
