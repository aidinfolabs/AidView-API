module namespace iati-olap = "http://kitwallace.me/iati-olap";
...
declare function iati-olap:sum-activities($activities as element(iati-activity)*, $date-from , $date-to) {
...
declare function iati-olap:sum-activities($activities as element(iati-activity)*, $year) {
...
declare function iati-olap:sum-activities($activities as element(iati-activity)*) {
   element transaction-summary {
      for $type in ("C","D","E","IF","IR","LR","PD","R","TB")  
      let $value := 
          sum($activities/*/value
...
declare function iati-olap:tree-group($activities as element(iati-activity)*, $dimensions as element(param)*, $corpus , $levels)  {
if (empty($dimensions))
then 
   element summary{
       element value { sum ($activities/@iati-ad:project-value) },
       element count {count($activities)},
       iati-olap:sum-activities($activities)
   }
else
...
let $code := iati-c:code-value($dimension/@name,$group-code,$corpus)
...
return
   ...
   then 
     ...
     element {concat(string-join($levels/facet,"-"),"-summary")} {
       ...
       iati-olap:sum-activities($group-activities)
   }
   else 
       iati-olap:tree-group($group-activities,$sub-dimensions,$corpus,$levels) 

};

declare function iati-olap:group($activities as element(iati-activity)*, $dimension as element(param)*, $corpus )  {
if (empty($dimension))
then 
   element summary{
       element value { sum ($activities/@iati-ad:project-value) },
...
declare function iati-olap:stats($activities, $dimension as element(param)*, $years, $corpus) {
let $facet := $dimension/@name/string()
let $group-exp := concat("distinct-values($activities/",$dimension/@path,")")
let $group-codes := util:eval($group-exp)
return
...
return
     element {$facet} {   
       ...
       for $year in $years 
       return  iati-olap:sum-activities($activities , $year )        
    }
};
