...
module namespace validate = "http://tools.aidinfolabs.org/api/validate";
...
declare variable $validate:errorTypes :=
  <errorTypes>
...
declare function validate:error($message as element(message)) as element(errorType)? {
  $validate:errorTypes/errorType[@name=$message/@errortype]
};

declare function validate:validation-analysis ($messages as element(message)*)  as element(validation) {
  let $messages := 
  ...
  let $errorAnalysis :=
    element errorTypes
      {
      for $errorType in $validate:errorTypes/errorType
      return
          element errorType {
             $errorType/@name,
...
declare function validate:activities-analysis($activities as element(iati-activity)*, $distribution-url as xs:string?) as element(div) {
...
declare function validate:path-analysis($activities as element(iati-activity)*, $path as xs:string) as element(div) {
...
declare function validate:assertion-validation($activity as element(iati-activity)) as element(message)* {
...
declare function validate:activity-schema-validation($activity as element(iati-activity)) as element(message)* {
...
declare function validate:activity-validation($activity as element(iati-activity)) as element(message)* {
  let $schema-errors := validate:activity-schema-validation($activity)
  let $assertion-errors := validate:assertion-validation($activity)
  return ($schema-errors,$assertion-errors)
};

declare function validate:validate-activity($activity as element(iati-activity)) as element(div){
  let $messages := validate:activity-validation($activity)
  let $report := validate:validation-analysis($messages)
  let $parsedXML := validate:nested-validation($activity,"",$activity)
  return
    <div>
      <table>
      <tbody>  
      {for $message in $report/message
       let $errorType := validate:error($message)
...
declare function validate:check($value as xs:string, $path as xs:string) as element(span)* {
...
declare function validate:nested-validation($activity as element(iati-activity), $path as xs:string, $element)  {
    if (exists ($element/(@*|node())))
    then 
     <table border="1">
        {if (exists($element/text()))
         then 
           let $check := validate:check($element,$path)
           return
               <tr  class="text">
                    <th ></th>
       ...
       let $check := validate:check($attribute,$path)
       return
         <tr  class="attribute">
       ...
       {for $node in $element/*
        ...
        let $check := validate:check($node,$path)
        return 
            <tr class="element">
                 <th>{name($node)}</th> 
                 <td>
                    { validate:nested-validation($activity,$path,$node)    } 
                 </td>
             </tr>       
        }
    </table>
...
declare function validate:rules-as-html() {
<div>
