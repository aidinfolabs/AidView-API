module namespace home = "http://tools.aidinfolabs.org/api/home-ui";
import module namespace config = "http://tools.aidinfolabs.org/api/config" at "config.xqm";
import module namespace load = "http://tools.aidinfolabs.org/api/load" at "load.xqm";
import module namespace validate-activity = "http://tools.aidinfolabs.org/api/validate-activity" at "validate-activity.xqm";
...
(: let $stats := doc(concat($config:base,"olap/",$query/corpus,"-stats.xml"))/stats   - when this olap data moved:)
let $stats := doc(concat($config:base,"cache/",$query/corpus,"/corpus-statistics.xml"))/stats
return 
(: --------- type=host  --------------------- :)
...
then
 ...
 then 
     let $host-data := doc(concat($config:data,"hosts.xml"))/hosts/host[@name=$query/host]  (: general host file - for comments :)
     let $host := $stats/host[download_url=$query/host]
     return  <div>
               <div class="nav">
...
then
 ...
 then 
     let $funder := $stats/funder[code=$query/funder]
...
(: --------------- type = activitySet   ---------------- :)  
  ...
  then
     let $activitySets :=  
          if (exists($query/src))
          then 
              $activitySets[download_url eq $query/src] 
          else 
      ...
      <div>
        ...
        <div>
         {wfn:paging(concat("?corpus=",$query/corpus,"&amp;type=activitySet&amp;host=",$query/host,"&amp;search=",$query/search),$query/start,$query/pagesize,count($activitySets))}
          {load:short-list-activitySets($selected-activitySets,$query/corpus)}
         </div>
     </div>
  ...
  <!--         <a href="?type=activitySet&amp;src={encode-for-uri($query/src)}&amp;mode=analysis">Analysis</a>  -->
   ...
   then  let $activities := collection($activity-collection)/iati-activity[@iati-ad:activitySet=$query/src][@iati-ad:live]       
         ...
         <div>
          <div class="nav">
           ...
           {validate-activity:activities-analysis($activities,concat("?mode=analysis&amp;corpus=",$query/corpus,"&amp;type=activitySet&amp;src=",encode-for-uri($query/src),"&amp;id="))}
         </div>
   ...
   then  let $activities := collection($activity-collection)/iati-activity[@iati-ad:activitySet=$query/src][@iati-ad:live]       
         return 
            ...
            {validate-activity:path-analysis($activities,$query/id)}
            </div>
   ...
   then  let $activities := collection($activity-collection)/iati-activity   
         ...
         <div>
          <div class="nav">
           ...
           {validate-activity:activities-analysis($activities,concat("?mode=analysis&amp;corpus=",$query/corpus,"&amp;type=activitySet&amp;id="))}
         </div>
...
(: ------  type = activity ---------------- :)
       ...
       <div>
         ...
         <div>
              { validate-activity:validate-activity($activity)}
         </div>
       </div>
 ...
 (:  --------- code and corpus -------------  :)
   ...
   then 
    <div>
         ...
         <div class="body">
         {validate-activity:rules-as-html()}
         </div>
       </div>
   ...
   then 
      <div>
          ...
          <div>
            <div>Number of activitySets {count($activitySets)}</div>
            <div>Number of ckan packages {count($activitySets[package])}</div>
            <div>Number of activitySets loaded {count($activitySets[download-modified])}</div>
  ...
  then  <div>
   ...
   then 
      let $corpus := doc(concat($config:system,"corpusindex2.xml"))//corpus[@name=$query/corpus]
      return 
      <div>
          <div class="nav">
