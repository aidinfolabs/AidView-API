module namespace admin-validate-ui = "http://tools.aidinfolabs.org/api/admin-validate-ui";
...
declare function admin-validate-ui:validate-content($query as item(), $script as xs:string ) {
...
(: download activitySet from a URL :)
  if ($query/mode="download" and exists($query/src))
       then 
          ...
          let $download := load:download-url($query/src, $activitySets, $query/corpus, $pipeline)
          return 
   ...
   then  let $activities := collection($activity-collection)/iati-activity[@iati-ad:activitySet=$query/src]     
         ...
         <div>
          <div class="nav">
           ...
           {validate-activity:activities-analysis($activities,concat("?mode=analysis&amp;type=activitySet&amp;src=",encode-for-uri($query/src),"&amp;id="))}
         </div>
