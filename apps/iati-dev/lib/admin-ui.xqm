module namespace admin-ui = "http://tools.aidinfolabs.org/api/admin-ui";
...
(: ------  type = code - master ---------------- :)
 ...
 (:  --------- code and corpus -------------  :)
  ...
  then let $zip := zipapp:zip(doc("/db/apps/iati-dev/system/resources.xml")//resource[empty(@ignore)]/@path , replace(concat("iati-dev-",current-dateTime()),":","-"))
       return
